...
from jobparser.items import instaposts
...
class InstagramSpider(scrapy.Spider):
    ...
    posts={} # Словарь постов. Будем складировать вывод
    ...
    def parse_user(self, response: HtmlResponse, user):
        ...
        yield response.follow(self.make_graphql_url(user_vars), #Форимруем ссылку на пользователя с помощью graphql
                              callback=self.parse_posts, #4-ый шаг. Сюда записываем функцию чтения постов
    ...
    def parse_followers(self, response: HtmlResponse, user_vars, user):
        ...
        if self.followers.get(user) and self.followers.get(user).get('count') == len(self.followers.get(user).get('followers')):
            pass #Здесь передаем список в pipeline


    #Собираем информацию о постах
    def parse_posts(self, response: HtmlResponse, user_vars, user):
        data = json.loads(response.body)
        # Проверяем повторно ли обращаемся к posts
        list_posts = data.get('data').get('user').get('edge_owner_to_timeline_media').get('edges') # идём по Path-y
                                                                                                # к контенту
        self.posts = {'posts': list_posts[:10]} # делаем срез
        yield instaposts(posts=self.posts)
